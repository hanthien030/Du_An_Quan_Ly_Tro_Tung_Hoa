...
function renderGrid(rooms) {
  ...
  grid.innerHTML = rooms.map(r => {
    ...
    const days = r.days_remaining != null
      ? `<div class="small text-muted">${r.days_remaining} ngày</div>` : '';
    return `
...
async function showDetail(id) {
  try {
    ...
    document.getElementById('btn-edit-detail').onclick = () => { detailModal.hide(); openModal(r); };
...
async function saveRoom() {
  ...
  if (!body.so_phong) { showToast('Nhập số phòng', 'warning'); return; }
  try {
    if (id) {
